...
def show_fitting(adc_map, s0_map, data, bval, selected_model, n_basal, n_b_val):
    ...
    # image_plot = ax[0].imshow(np.fliplr(rotate(current_slice, 270)), extent=(0, 128, 128, 0))
    ax[0].imshow(np.fliplr(rotate(current_slice, 270)), extent=(0, 128, 128, 0))
    ax[1].text(
        0.5,
        0.5,
    ...
    # Set up the slider [0.15, 0.1, 0.7, 0.03]    [0.1, 0.25, 0.0225, 0.63]  [0.25, 0.1, 0.65, 0.03]
    slider_ax = fig.add_axes([0.05, 0, 0.3, 0.03], facecolor="lightgray")
    slice_slider = Slider(
        slider_ax,
        "Slice",
        0,
    ...
    def update_slice(val):
        ...
        current_slice = adc_map[:, :, int(slice_slider.val), initial_dir]

        # image_plot.set_data(np.fliplr(rotate(current_slice, 270)), extent=(0, 128, 128, 0))
    ...
    def onclick(event):
        ...
        try:
            ...
            # y_data = np.append(
            #     original_data[x, y, 0, :3],
            #     original_data[x, y, 0, 3:5],
            # )

            x_data = np.append(bvalues[:n_basal], bvalues[n_basal : n_basal + n_b_val])

            print(slice_slider.val)

            y_data = np.append(
                original_data[x, y, int(slice_slider.val), :n_basal],
                original_data[x, y, int(slice_slider.val), n_basal : n_basal + n_b_val],
            )
